...
export class EvmService {
  ...
  public async addBlockListener(
    listener: (block: BlockWithTransactions) => void,
    includeCurrent = true,
  ): Promise<void> {
    ...
    if (
      this.listeners
        ...
        .listenerCount(eventName) === 1
    ) {
      if (includeCurrent)
        this.factory.provider
          .getBlockNumber()
          .then(async (n) =>
            listener(await this.factory.provider.getBlockWithTransactions(n)),
          );

      this.factory.provider.on('block', async (blockNumber) => {
        const latestBlock =
          await this.factory.provider.getBlockWithTransactions(blockNumber);
  ...
  public static getAbiEncodedArguments(
    argTypes: string[],
    argValues: any[],
  ): Uint8Array {
